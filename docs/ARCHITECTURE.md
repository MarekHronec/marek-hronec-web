...
| `compliance` | Compliance | Topic | EU cloud compliance, DORA, NIS2, GDPR, sovereign cloud |
...
The `npm run build` script chains two commands: `astro build` (produces `dist/`) followed by `npx pagefind --site dist` (generates the full-text search index at `dist/pagefind/`). Both outputs are included in the Pages artifact and served as static files. No server runtime, no environment variables, no secrets required.
...
**Static pre-rendering.** All pages — 6 static routes plus one per content entry (3 case studies + 54 KB articles = 63 total for current content) — are built at deploy time. Time to first byte is the CDN edge latency. There is no server to slow down under load.
