...
export function planRecovery(selection: RecoverySelection) {
  ...
  else if(answers.rto) add('Test whether backup and rebuild meet the time budget','A longer RTO can make restore-based recovery practical. Do not assume large datasets or unavailable capacity can be recovered within one day.');
  ...
  else if(answers.rpo) add('Keep a usable recovery point within 24 hours','Allow for failed jobs, copy delays and consistency checks. Retention and restore tests matter as much as the nominal backup interval.');
  ...
  if(answers.scope==='region') {
    ...
    else if(answers.placement) gaps.push(answers.placement==='restricted'?'Your region-outage scope conflicts with a single-region restriction. In-region redundancy cannot keep operating through loss of that whole region. Resolve the constraint or document the accepted interruption.':'Confirm a permitted recovery location before claiming readiness for a region outage.');
  }
  ...
  if(answers.evidence) add(answers.evidence==='tested'?'Keep the recovery evidence current':'Run a representative recovery exercise',
  ...
  return {answered,complete:answered===RECOVERY_QUESTIONS.length,priorities,gaps,summary:answered!==RECOVERY_QUESTIONS.length?'Complete the remaining questions to finish the recovery brief.':gaps.length?'Resolve the open requirements before selecting a recovery pattern.':'Use these priorities to compare designs and plan your next recovery exercise.'};
}
