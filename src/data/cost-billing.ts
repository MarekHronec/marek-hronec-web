...
export function billingExample(key:CostKey,input:BillingInputs={}):CostExample & { summary:string; accrued?:number; remaining?:number } {
if(key==='idle'){
...
summary:'Release on day '+hour/24+': '+(mode==='hourly'?'avoid '+remaining/24+' days of charges for two vessels.':mode==='monthly'?"this month’s capacity fee stays payable.":'the capacity commitment stays payable for its term.')+' '+(input.storage?'Unused storage is also released.':'Storage keeps billing.'),
...
if(key==='scaling'){const days=bounded(input.days,10,30);return {...COST_EXAMPLES.scaling,summary:days+' peak days at €20/day extra. Releasing the extra capacity stops future charges.',rows:[{label:'Baseline & retained services',before:800,after:800},{label:'Extra capacity for the peak',before:0,after:2*days*10}],assumption:'2 extra vessels × '+days+' days × €10/day. Only the peak duration changes; the baseline remains all month.',outcome:'The peak adds €'+(2*days*10)+' this month. Releasing the extra capacity stops future charges, without refunding completed days.'};}
if(key==='transfer'){const gb=bounded(input.gb,1000,10000);return {...COST_EXAMPLES.transfer,summary:gb.toLocaleString('en-IE')+' GB at €0.20/GB. Once transferred, that usage stays on the bill.',rows:[{label:'Existing services',before:600,after:600},{label:'Additional outbound transfer',before:0,after:gb*.2}],assumption:gb+' GB × an invented €0.20/GB. Usage-based: elapsed hours do not determine this charge. No free allowance or other fees included.',outcome:'This shipment adds €'+(gb*.2)+'. Stopping a resource afterwards does not undo data already transferred.'};}
return {...COST_EXAMPLES[key],summary:'Provider fee rises; team hours fall. Freed time is not automatically a cash saving.'};
}
