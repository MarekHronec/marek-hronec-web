...
export function remarkCallouts() {
  return function (tree) {
    // Restore unrecognised text directives (e.g. :2022) to literal colon-name text
    // so ISO cert notation like `ISO 27001:2022` is not eaten by remark-directive
    visit(tree, 'textDirective', function (node, index, parent) {
      if (!parent) return;
      parent.children.splice(index, 1, { type: 'text', value: `:${node.name}` });
      return [SKIP, index];
    });
